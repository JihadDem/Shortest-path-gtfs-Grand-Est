...
SOURCE_SPACE = '../../gtfs/'
EXTENSION = '.txt'
TIMETABLE_HEADERS = ['from', 'to', 'departure','arrival', 'travel_time', 'route', 'route_type']
...
def join_csv_on(file1, keys1, file2, keys2, output_file):
    # sorting file1
    csvsort(BASE_PATH + file1, keys1, output_filename=BASE_PATH + WORKING_SPACE +'./file1.sorted' + EXTENSION, has_header=True)
    # sorting file2
    csvsort(BASE_PATH + file2, keys2, output_filename=BASE_PATH + WORKING_SPACE +'./file2.sorted' + EXTENSION, has_header=True)
    # merging
    file1_reader = open(BASE_PATH + WORKING_SPACE +'./file1.sorted' + EXTENSION, 'r')
    file2_reader = open(BASE_PATH + WORKING_SPACE +'./file2.sorted' + EXTENSION, 'r')
...
print('Suppressions des stops (stops.txt) qui ne sont pas dans Strasbourg...')
...
csvsort(BASE_PATH + WORKING_SPACE + 'stops-stop_times-trips-routes' + EXTENSION,['trip_id', 'departure_time'],
...
csvsort(BASE_PATH + SOURCE_SPACE + 'transfers' + EXTENSION, [0],output_filename=BASE_PATH + WORKING_SPACE + 'transfers' + EXTENSION)
exec_time.out()

print('Tri des arcs par stop (stop_id)')
csvsort(BASE_PATH + WORKING_SPACE + 'arcs_transport' + EXTENSION, [0],output_filename=BASE_PATH + WORKING_SPACE + 'arcs_transport' + EXTENSION)
exec_time.out()
...
for arc in arcs_transport:
    try:
        ...
        error += 1  # j'ai trouvé le cas où deux lignes étaient sur une...
