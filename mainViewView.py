...
class View(QMainWindow):
    ...
    class UI():
        def __init__(self):
            ...
            self.queryModeSelectorLayout.addWidget(self.queryModeTramwayCable)
    ...
    def getSelectedModes(self):
        modes = []
        if self.ui.queryModeTramway.checkState() == Qt.Checked: modes += [1]
        if self.ui.queryModeTrain.checkState() == Qt.Checked: modes += [2]
        if self.ui.queryModeBus.checkState() == Qt.Checked: modes += [3]
        if self.ui.queryModeTramwayCable.checkState() == Qt.Checked: modes += [4]
        return modes
    ...
    def fillResultTable(self, stop_times, stop_names, route_types, route_names, route_colors):
        route_type_str = { 0: 'Tramway', 1: 'Train', 2: 'Bus', 3: 'Tramway cable'}
        n = len(stop_times)
    ...
    def disableModes(self, allowedModes):
        ...
        self.ui.queryModeTrain.setDisabled(1 not in allowedModes)
        self.ui.queryModeBus.setDisabled(2 not in allowedModes)
        self.ui.queryModeTramwayCable.setDisabled(3 not in allowedModes)
        self.ui.queryModeFerry.setDisabled(4 not in allowedModes)
        self.ui.queryModeMetro.setDisabled(5 not in allowedModes)
