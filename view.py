...
class View:
    def __init__(self, controller, qApp):
        ...
        for cbox in [self.bus, self.tram, self.train]:QObject.connect(cbox, SIGNAL"stateChanged(int)"), self.refresh)
        for cbox in [self.start, self.end]:QObject.connect(cbox, SIGNAL("currentIndexChanged(int)"), self.on_stop_selected)
