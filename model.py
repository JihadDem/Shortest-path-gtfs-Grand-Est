...
class Model:
    def __init__(self,controller):
        ...
        self.TIMETABLE_HEADERS = ['from', 'to', 'departure','arrival', 'travel_time', 'route', 'route_type']
        self.STOPSMAP_HEADERS = ['id', 'name', 'lat', 'lon', 'route_type']
        self.MODES = {1: 'tram', 2: 'train', 3: 'bus'}
    ...
    def check_workspace(self):
        return self.files_exists(['stops.txt', 'stop_times.txt', 'transfers.txt', 'routes.txt', 'trips.txt'])

    def check_workspace_ready(self):
        all = ['condensed.txt','condensed-tram.txt','condensed-train.txt','condensed-bus.txt','condensed-tram-train.txt','condensed-tram-bus.txt','condensed-train-bus.txt','condensed-tram-train-bus.txt','alpha_beta.txt','alpha_beta-tram.txt','alpha_beta-train.txt','alpha_beta-bus.txt','alpha_beta-tram-train.txt','alpha_beta-tram-bus.txt','alpha_beta-train-bus.txt','alpha_beta-tram-train-bus.txt']
        return self.files_exists(['time_expanded-tram.txt','time_expanded-train.txt','time_expanded-bus.txt','time_expanded-tram-train.txt','time_expanded-tram-bus.txt','time_expanded-train-bus.txt','time_expanded-tram-train-bus.txt','stops-tram.txt','stops-train.txt','stops-bus.txt','stops-tram-train.txt','stops-tram-bus.txt','stops-train-bus.txt','stops-tram-train-bus.txt']),

    def open_csv(self, filename):
        f = open(self.get_conf('WORKSPACE') + '/' + filename + self.get_conf('EXTENSION'))
    ...
    def join_csv_on(self, file1, keys1, file2, keys2, output_file):
        ...
        # sorting file1
        csvsort(workspace_path + file1 + extension, keys1, output_filename=workspace_path +'/file1.sorted' + extension, has_header=True)
        # sorting file2
        csvsort(workspace_path + file2 + extension, keys2, output_filename=workspace_path +'/file2.sorted' + extension, has_header=True)
        # merging
    ...
    def build_time_expanded_model(self):
        ...
        self.join_csv_on('stops-reduced', [0],'stop_times', [3], 'stops-stop_times')

        print('Jointure des heures d\'arrêts résultants avec les voyages (trips.txt)')
        self.join_csv_on('trips', [2], 'stops-stop_times',[4], 'stops-stop_times-trips')

        print('Jointure des heures d\'arrêts résultants avec les routes (routes.txt)')
        self.join_csv_on('routes', [0], 'stops-stop_times-trips',[0], 'stops-stop_times-trips-routes')

        print('Tri des heures d\'arrêts par voyage (trip_id) et heure de départs (departure_time)...')
        csvsort(workspace_path + 'stops-stop_times-trips-routes' + extension,['trip_id', 'departure_time'], output_filename=workspace_path + 'jointure' + extension)
        ...
        csvsort(workspace_path + 'transfers' + extension, [0],output_filename=workspace_path + 'transfers' + extension)

        print('Tri des arcs par stop (stop_id)')
        csvsort(workspace_path + 'arcs_transport' + extension, [0], output_filename=workspace_path + 'arcs_transport' + extension)
    ...
    def build_condensed_model_from_time_expanded_model(self):
        ...
        workspace_path = self.get_conf('WORKSPACE') + '/'
        extension = self.get_conf('EXTENSION')
        csvsort(workspace_path + 'time_expanded-tram-train-bus' + extension,['from', 'to'], output_filename=workspace_path + 'beta-tram-train-bus' + extension, has_header=True)
        arcs = self.open_csv('beta-tram-train-bus')
    ...
    def build_cuboid_from_model(self, model_type):
        ...
        for combi in cuboid:
            outputs[combi].close()
        print(str(len(cuboid)) + ' ' + model_type +' models have been generated')
    ...
    def bellman(self, route_types, start_node, end_node, start_time=None):
        ...
        stops = self.open_csv('stops-tram-train-bus')  # + mode_prefix)
        for stop in stops:
...
def build_successors_list(self):
        ...
        for source in ['time_expanded-tram','time_expanded-train','time_expanded-bus', 'time_expanded-tram-train','time_expanded-tram-bus', 'time_expanded-train-bus','time_expanded-tram-train-bus','condensed-tram','condensed-train','condensed-bus','condensed-tram-train','condensed-tram-bus','condensed-train-bus','condensed-tram-train-bus']:
            print(workspace_path+source+'.db')
            csvsort(workspace_path + source + extension,['from', 'departure'], output_filename=workspace_path + source + 'sort' + extension)
